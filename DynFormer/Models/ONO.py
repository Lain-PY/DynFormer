...
class ONOBlock(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("feature_cov", torch.tensor([[torch.inf for _ in range(psi_dim)] for _ in range(psi_dim)]))
        # self.register_buffer("feature_cov", torch.empty(0))
        if not self.orth:
    ...
    def forward(self, x, fx , T = None):
        ...
        if self.orth:
            ...
            if self.training:
                ...
                with torch.no_grad():
                    if torch.equal(self.feature_cov, torch.tensor([[torch.inf for _ in range(self.psi_dim)] for _ in range(self.psi_dim)]).to(x.device)):
...
class ONOgateBlock(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("feature_cov", torch.tensor([[torch.inf for _ in range(self.psi_dim)] for _ in range(self.psi_dim)]))
        self.register_buffer("feature_covq", torch.tensor([[torch.inf for _ in range(self.psi_dim)] for _ in range(self.psi_dim)]))
        if not self.orth:
            #self.bn = nn.BatchNorm1d(psi_dim)
...
class ONO(nn.Module):
    def __init__(self, model_config, device):
        ...
        self.attn_dropout=model_config.attn_dropout  # 0
        self.attn_type=model_config.attn_type
        self.n_head=model_config.n_head
        self.Time_Input=model_config.Time_Input  # False
        self.act=model_config.act # 'gelu'
        self.mlp_ratio=model_config.mlp_ratio # '1'
        self.orth=model_config.orth # true
        self.psi_dim=model_config.psi_dim # 64
        self.momentum=model_config.momentum # 0.9
        ...
        self.preprocess = MLP(self.fun_dim + self.space_dim , self.n_hidden * 2, self.n_hidden * 2, n_layers=0, res=False, act=self.act).to(device)
        if self.Time_Input:
            self.time_fc = nn.Sequential(nn.Linear(self.n_hidden, self.n_hidden), nn.SiLU(),nn.Linear(self.n_hidden, self.n_hidden)).to(device)
         
        self.blocks = nn.ModuleList([ONOBlock(num_heads=self.n_head, hidden_dim=self.n_hidden, 
                                              dropout=self.dropout, attention_dropout=self.attn_dropout,
                                              act=self.act, attn_type=self.attn_type, 
                                              mlp_ratio=self.mlp_ratio, orth=self.orth, momentum = self.momentum,
                                              psi_dim=self.psi_dim,out_dim = self.out_dim * self.out_steps,
    ...
    def forward(self, x: torch.Tensor, static_data: list) -> torch.Tensor:
        ...
        for t in range(self.seq_len):
            ...
            if current_input is not None:
                x = torch.cat((x_pos, current_input), -1)
...
def psd_safe_cholesky(A, upper=False, out=None, jitter=None):
	...
	try:
		...
		if jitter is None:
			# Increased jitter from 1e-6 to 1e-5 for better numerical stability
