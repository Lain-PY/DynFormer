...
def count_flops_and_params(model, input, static_data, truth_data):
    flops, params = profile(model, inputs=(input, static_data, truth_data))
    flops, params = clever_format([flops, params], "%.3f")
    return flops, params

def generate_coor_input(data, config, train):
    """
    Generate coordinate inputs for models based on dimensionality
    
    Args:
        data: Original Dataset
        
    Returns:
        Coordinate input tensor of appropriate dimension
    """
...
class ModelCreator:
    ...
    def create_or_load_model(self, model_config=None, model=None, checkpoint_path=None):
        ...
        if checkpoint_path is not None:
            print(f"Loading checkpoint: \n {checkpoint_path} for {model_name}...")
